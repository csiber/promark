...
export function Header() {
  ...
  return (
    <>
      <div
        className={`absolute w-full h-full pointer-events-none ${scrolled ? "backdrop-blur-xl" : "bg-white/0 dark:bg-black/0"
          }`}
      ></div>
      <Navbar
        ...
        className={`${_config.navigation.ui.style?.bg === "none" &&
          `${scrolled
      ...
      >
        <Container fluid>
          ...
          <Navbar.Offcanvas
            id={`offcanvasNavbar-expand-${_config.navigation.ui.style?.expand || "lg"
              }`}
            aria-labelledby={`offcanvasNavbarLabel-expand-${_config.navigation.ui.style?.expand || "lg"
              }`}
            placement="end"
          >
            <Offcanvas.Header closeButton>
              <Offcanvas.Title
                id={`offcanvasNavbarLabel-expand-${_config.navigation.ui.style?.expand || "lg"
                  }`}
              >
                {_config.meta.title}
              </Offcanvas.Title>
...
export function Footer() {
  ...
  return (
    <div className="pt-10">
      <footer className="border-t border-neutral-200 dark:border-neutral-700 flex flex-col w-full transition-all duration-100 ease-in-out items-center justify-center">
            ...
            >
              {_config.footer?.ui.logo?.url && (
                <Image
                  ...
                  width={40}
                  height={40}
                  className={`w-[40px] h-[40px] hover:scale-[102%] hover:shadow-lg active:scale-[98%] active:shadow transition-all duration-300 ease-in-out ${_config.footer?.ui.logo?.css || ""
                    }`}
                />
              )}
